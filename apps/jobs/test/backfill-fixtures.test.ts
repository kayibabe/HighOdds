...
describe("backfillFixtures", () => {
  it("fetches and ingests exactly one date per day in [from, to), oldest first", async () => {
    ...
    const alreadyFetched = async () => false;

    const result = await backfillFixtures(from, to, client, ingest, alreadyFetched);

    expect(requestedDates).toEqual(["2026-01-01", "2026-01-02", "2026-01-03"]);
    expect(ingestedBatches).toHaveLength(3);
    expect(result).toEqual({ datesProcessed: 3, skipped: 0, ingested: 3, rejected: 0 });
  });
  ...
  it("propagates errors from the client without swallowing progress already returned by the caller", async () => {
    ...
    await expect(backfillFixtures(from, to, client, ingest, alreadyFetched)).rejects.toThrow("quota exceeded");
    expect(calls).toBe(2);
  });

  it("skips dates that were already fetched instead of re-spending quota on them", async () => {
    const from = new Date("2026-01-01T00:00:00.000Z");
    const to = new Date("2026-01-04T00:00:00.000Z");
    const requestedDates: string[] = [];
    const client = { getPaged: async (_endpoint: string, query: Record<string, string | number | undefined>) => { requestedDates.push(String(query.date)); return []; } };
    const ingest = async () => ({ ingested: 0, rejected: 0 });
    const alreadyFetched = async (date: string) => date === "2026-01-02";

    const result = await backfillFixtures(from, to, client, ingest, alreadyFetched);

    expect(requestedDates).toEqual(["2026-01-01", "2026-01-03"]);
    expect(result).toEqual({ datesProcessed: 2, skipped: 1, ingested: 0, rejected: 0 });
  });
});
